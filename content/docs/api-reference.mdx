...
defense-in-depth CWE-22 path traversal protection against malicious filenames in
archive writes.

**Source:** `workspace-server/internal/handlers/container_files.go`
**Handler:** `TemplatesHandler.WriteFile` → `copyFilesToContainer`
**Related PRs:** [#1267](https://github.com/Molecule-AI/molecule-core/pull/1267),
...
**Error codes for path traversal:**

| Status | Condition | Response |
|--------|-----------|----------|
| `400` | Traversal attempt detected | `{"error": "invalid path"}` |
| `404` | Workspace not found | `{"error": "workspace not found"}` |
| `500` | Docker unavailable | `{"error": "failed to write file: docker not available"}` |
