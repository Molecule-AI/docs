...
- **Fresh tenant provisioning unblocked — staging canary now greens again** ([`molecule-controlplane` #501](https://github.com/Molecule-AI/molecule-controlplane/pull/501)): tenant boot was wedging because Ubuntu 24.04.2's `docker.io` package update added a debconf prompt ("Automatically restart Docker daemon?") to its postinst, and `apt-get install -y` only suppresses apt's own y/n confirmation — dpkg's debconf prompts honor `DEBIAN_FRONTEND`, which wasn't set. With nothing set, dpkg fell back to dialog/whiptail, found no terminal on user-data, and **blocked forever**. cloudflared was three steps after `install_apt_base` so the tunnel had 0 connectors → CF error 1033 → canary's 15-min TLS-readiness wait timed out. Fixed by exporting `DEBIAN_FRONTEND=noninteractive` at the top of user-data + pre-seeding `docker.io/restart=false` as defense-in-depth.
- **Cross-CP destructive tunnel-delete stopped** ([`molecule-controlplane` #499](https://github.com/Molecule-AI/molecule-controlplane/issues/499) / [#500](https://github.com/Molecule-AI/molecule-controlplane/pull/500)): the staging CP's orphan-tunnel sweep was treating tunnels created by the **production** CP as orphans (because they didn't appear in staging's `organizations` table) and deleting them. 3 prod tenant tunnels were removed at 2026-05-06T03:15Z. Detector now requires positive proof of CP-ownership via this CP's `tenant_resources` claim rows — same SSOT shape as `reference_tenant_resources_audit`. Verified to fail on old code (`CROSS-ENV LEAK: DeleteTunnel called for unclaimed tunnel`). Pairs with #501 above: existing tunnels stop being deleted, fresh provisions now actually boot.
...
- **In-CP orphan-tunnel sweep cron** ([`molecule-controlplane` #3001](https://github.com/Molecule-AI/molecule-controlplane/issues/3001) / [#502](https://github.com/Molecule-AI/molecule-controlplane/pull/502)): the orphan-tunnel cleanup that previously lived in `molecule-core`'s sweep workflow is now scheduled inside the CP itself, scoped to this CP's claims (matches the #500 detection invariant). Operator no longer needs the `molecule-core` repo's GitHub Actions to be healthy for tunnel hygiene.
- **Typed `EventType` registry — 18 producers migrated** ([RFC #2945 PR-B-1](https://github.com/Molecule-AI/molecule-core/pull/2993)): every WebSocket event emit site now uses the typed constants from the registry introduced yesterday in #2965, replacing 18 inline string literals. No on-the-wire change; foundation for the full Agent-OS-shaped messaging refactor in RFC #2945.
