...
- **CWE-22 path traversal protection** — `copyFilesToContainer` now validates file paths at the tar-archive-write boundary, preventing writes outside the workspace volume mount. Handler-level `validateRelPath` is defense-in-depth — do not remove. Affects `PUT /workspaces/:id/files/*path` and `DELETE /workspaces/:id/files/*path`.
  ([#1267](https://github.com/Molecule-AI/molecule-core/pull/1267),
  [#1270](https://github.com/Molecule-AI/molecule-core/pull/1270),
  [#1281](https://github.com/Molecule-AI/molecule-core/pull/1281))
...
- **Workspace File Copy API reference** — full reference for `PUT /workspaces/:id/files/*path` including CWE-22 path traversal protection, error codes, and curl examples.
