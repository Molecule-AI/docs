...
- **Docs queue maintenance**: self-hosted Docker deployment guide, workspace ability flags (`broadcast_enabled` / `talk_to_user_enabled`) API reference, MCP server broadcast tool, CWE-78 `expandWithEnv` regression fix, OFFSEC-006 advisory, and OFFSEC-003 boundary-escaping fix all prepared in open pull requests — pending CI clearance and merge. (`docs` [#40](https://git.moleculesai.app/molecule-ai/docs/pulls/40)–[#49](https://git.moleculesai.app/molecule-ai/docs/pulls/49))
